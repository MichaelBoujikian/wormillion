...
- **Filler words are not letters.** "Mount", "Lake", "the", "River"… are stripped before any letter rule ("has a T", "starts with M", "ends in", "double letter"): the user explicitly did not want "Mount Kilimanjaro" to count as having a T. The two length rules are the deliberate exception — they count every spelling in full, because length is what the player types. `variantsOf()` vs `spellingsOf()` in `promptBank.js`.
