...
- **Filler words are not letters.** "Mount", "Lake", "the", "River"… are stripped before any letter rule ("has a T", "starts with M", "ends in", "double letter"): the user explicitly did not want "Mount Kilimanjaro" to count as having a T. Words that ARE the name stay even if they mean "lake" in Gaelic: "Loch Ness" starts with L, "Saint Lucia" starts with S (`LETTER_FILLER` = the matcher's filler minus loch/lough/llyn/saint/st). The two length rules are the deliberate exception — they count every spelling in full AND the shortest form the matcher accepts, because length is what the player types ("Mount Kilimanjaro" is long; "Fuji" and "Ness" are short). `variantsOf()` vs `spellingsOf()` in `promptBank.js`.
