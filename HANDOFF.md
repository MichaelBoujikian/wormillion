...
- **The city cohort excludes national capitals, and every city prompt says so in the sentence** — the plain one in full ("Name a city that isn't a national capital."), every narrowed one through the noun (`NOUN.city = 'non-capital city'`: "Name a non-capital city in the Caribbean."). The badge reads "City (not a capital)" as well; Paris on a city round gets "Paris is a capital city — this round wants a non-capital city" (`wantsPhrase` in `promptBank.js`, used by `ui.js`). The user asked for the wording in the question itself, not just the badge. Don't add capitals to the city list to be generous — the prompt would become a lie. `size` is the city-proper population, consistently; the figures were written from memory like the flags, so fix the specific one a player trips on.
